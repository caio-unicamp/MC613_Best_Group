        ...
        if reset_n = '0' then
            ...
            if current_state = START then   -- Aguardando o início do jogo
                ...
                if key(0)='0' or key(1)='0' or key(2)='0' or key(3)='0' then
                    ...
                    if random_seed = 0 then    dx <= 3;  dy <= 3;
                    elsif random_seed = 1 then dx <= -3; dy <= 3;
                    elsif random_seed = 2 then dx <= 3;  dy <= -3;
                    else                       dx <= -3; dy <= -3;
                    end if;
                end if;
                ...
                if not(key(3) = '0' and key(2) = '0') then    -- Se aperta os dois botões ao mesmo tempo fica parado
                    ...
                    if key(2) = '0' and pos_p1_x < SCREEN_W - P_WIDTH - 1 then -- Direita
                        if pos_p1_x > SCREEN_W - P_WIDTH - 1 - SPEED then  -- Pra não passar do máximo
                ...
                if not(key(1) = '0' and key(0) = '0') then    -- Se aperta os dois botões ao mesmo tempo fica parado
                    ...
                    if key(0) = '0' and pos_p2_x < SCREEN_W - P_WIDTH - 1 then -- Direita
                        if pos_p2_x > SCREEN_W - P_WIDTH - 1 - SPEED then  -- Pra não passar do máximo
                ...
                -- Passou do Jogador 1 (Ponto para o Jogador 2)
                    ...
                    b_x <= 304; 
                    b_y <= 224;
                    current_state <= IDLE; -- Pausa o jogo esperando novo saque
